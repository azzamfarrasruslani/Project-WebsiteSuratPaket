...
class SerahTerimaModel extends Model
{
    public function getPenyerahanTerimaan($search, $order, $orderType, $limit, $offset)
    {
        ...
        if ($search) {
            $query .= " AND (st.posisi LIKE '%$search%' OR st.status_barang LIKE '%$search%' )";
        }
    ...
    public function insertKurir($ekspedisi)
    {
        $sql = "INSERT INTO kurir (ekspedisi) VALUES (?)";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("s", $ekspedisi);
        if ($stmt->execute()) {
            return $stmt->insert_id;
        } else {
            return false;
    ...
    public function updateKurir($id_kurir, $ekspedisi)
    {
        $sql = "UPDATE kurir SET ekspedisi = ? WHERE id_kurir = ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("si", $ekspedisi, $id_kurir);
        return $stmt->execute();
    }
    ...
    {
        if ($waktu_penyerahan === null || $waktu_penyerahan === '') {
            $query = "UPDATE serah_terima 
                      SET posisi = ?, status_barang = ?, waktu_penerimaan = ?, id_barang = ?, id_kurir = ?, id_pemilik = ?, id_security = ?
                      WHERE id_serah_terima = ?";
            $stmt = $this->conn->prepare($query);
            $stmt->bind_param("sssiisii", $posisi, $status_barang, $waktu_penerimaan, $id_barang, $id_kurir, $id_pemilik, $id_security, $id_serah_terima);
        } else {
            $query = "UPDATE serah_terima 
    ...
    public function updateBarangWithoutFoto($id_barang, $jenis_barang, $no_resi)
    {
        $sql = "UPDATE barang SET jenis_barang = ?, no_resi = ? WHERE id_barang = ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("ssi", $jenis_barang, $no_resi, $id_barang);
        return $stmt->execute();
    }

    // public function updateSerahTerimaWithoutPenyerahan($id_serah_terima, $posisi, $status_barang, $waktu_penerimaan, $id_barang, $id_kurir, $id_pemilik, $id_security)
    // {
    //     $query = "UPDATE serah_terima 
    //           SET posisi = ?, status_barang = ?, waktu_penerimaan = ?, id_barang = ?, id_kurir = ?, id_pemilik = ?, id_security = ?
    //           WHERE id_serah_terima = ?";
